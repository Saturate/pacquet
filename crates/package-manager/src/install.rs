...
mod tests {
    ...
    /// then on the success path `pnpm:stage importing_done` followed by
    /// `pnpm:summary`. On an early-error path (e.g. `NoLockfile`) only
    /// the leading events fire. This matches pnpm: context and the
    /// chosen import strategy go in the install header, the stage
